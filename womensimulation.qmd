...
saveRDS(simulations, file = "simulations_women.rds")
```

```{r}
women_sims <- readRDS("simulations_women.rds")
```
...
USA_medals_w <- vector("list", length = numSimulations)
...
top_indices <- order(USA_medals_w, decreasing = TRUE)[1:2]
top_indices
...
```{r}
print("Done with women gymnastics simulation")
```
