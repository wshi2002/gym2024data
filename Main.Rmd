...
## Introduction (WORK IN PROGRESS - Nidhi)
...
The 2024 Olympic Games will be held in Paris, France, and will include both a Men’s and Women’s Artistic Gymnastics event. Winning a medal in an Olympic gymnastics event is an honor for both the individual athlete and for the country that athlete represents; the medal brings a sense of national pride to citizens across the country and promotes the global recognition of the winning country. The USA is known for its men and women gymnastics teams, which have consistently earned medals in past Olympic games. Because of the high honor and importance of upholding the legacy of the United States in these competitions, it is important to carefully select the men and women that will represent our country as Team USA.
...
The objective of this project is to select the 5 athletes for each of men’s and women’s events that will optimize performance for Team USA across the 8 medal events for men and the 6 medal events for women.
...
```{r diff-quali-v-final-vis-women, fig.cap="\\label{fig:figs}Distribution of the Difficulty Score by Round and Apparatus for Women "}
...
```{r diff-world-v-nonworld-vis-women, fig.cap="\\label{fig:figs}Distribution of Difficulty scores by the Scope of Competition and Apparatus for Women"}
...
```{r Score-world-v-nonworld-vis-women, fig.cap="\\label{fig:figs}Distribution of Scores by Scope of Competition and Apparatus for Women"}
...
```{r penalty-dist-women, \\fig.cap="\\label{fig:figs}Distribution of Penalties by competition scope for Women"}
...
```{r penalty-dist-men, \\fig.cap="\\label{fig:figs}Distribution of Penalties by competition scope for Men}
...
```{r score-dist-women, \\fig.cap="\\label{fig:figs}Score Distribution for Women Athletes}
...
```{r score-dist-men, \\fig.cap="\\label{fig:figs}Score Distribution for Men Athletes}
...
```{r diff-quali-v-final-vis-men, fig.cap="\\label{fig:figs}Distribution of the Difficulty Score by Round and Apparatus for Men"}
...
```{r diff-world-v-nonworld-vis-men, fig.cap="\\label{fig:figs}Distribution of Difficulty scores by the Scope of Competition and Apparatus for Men"}
...
```{r Score-world-v-nonworld-vis-men, fig.cap="\\label{fig:figs}Distribution of Scores by Scope of Competition and Apparatus for Men"}
