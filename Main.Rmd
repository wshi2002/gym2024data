...
```{r score-dist-women-hist}

women_rounddist <- ggplot(data = df_women_c, aes(x = Score)) +
  geom_histogram(aes(fill = Round), alpha = 0.8) +
  labs(title = "Women",
       x = "Score",
       y = "Count") +
  theme_light() +
  ...
  #scale_fill_aaas()
  scale_fill_manual(values = c("AAFinal" = "#3e4a91", "final" = "#eb0608", "qual" = "#048b48", "TeamFinal" = "#611a75", "TeamQual" = "#0b8280"))

```
```{r score-dist-men-hist}

men_rounddist <- ggplot(data = df_men_c, aes(x = Score)) +
  geom_histogram(aes(fill = Round), alpha = 0.8) +
  labs(title = "Men",
       x = "Score",
       y = "Count") +
  theme_light() +
     # scale_fill_manual(values = c("AAFinal" = "#3e4a91", "final" = "#eb0608", "qual" = "#048b48", "TeamFinal" = "#611a75", "TeamQual" = "#0b8280"),
      #                  labels = c("Individual All-Around Final", "Individual Apparatus Final", "Individual Apparatus Qualification", "Team Final", "Team Qualification"))
  scale_fill_aaas(labels = c("Individual All-Around Final", "Individual Apparatus Final", "Individual Apparatus Qualification", "Team Final", "Team Qualification"))
#grid.arrange(women_rounddist, men_rounddist, ncol = 2, widths = c(2, 4))


```

```{r full-medal-dist, fig.cap = "Score Distribution by Round"}
grid.arrange(women_rounddist, men_rounddist, ncol = 2, widths = c(2, 4))
```
...
ggplot(data = w_team_usa, aes(y= clean_name, x = Rank, color = competition_scope))+
  geom_point()+
  theme_light()+
  scale_color_aaas()+
  labs(
...
```
```{r range-of-rank-m, fig.cap = "Range of Rank at all Competitions for US Mens Team"}

ggplot(data = m_team_usa, aes(y= clean_name, x = Rank, color = competition_scope))+
  geom_point()+
  theme_light()+
  scale_color_aaas()+
  labs(
...
```
```{r dif-vs-ex-score-usa-women, fig.cap = "Difference between Difficulty and Exectution Scores for team USA women"}
#pivot longer for team USA women
usa_women_pivot_longer <- pivot_longer(w_team_usa, cols = c(D_Score, E_Score), names_to = "D_E_labs", values_to = "D_E_Score")


ggplot(data = usa_women_pivot_longer, aes( x = clean_name, y = D_E_Score, fill = D_E_labs))+
  geom_bar(stat = "identity",
           position = position_dodge(width = 0.8))+
  theme_light()+
  scale_fill_aaas(labels = c("Difficulty Score", "Execution Score"))+
  labs(
    x = "Team USA Athletes",
    y = "Score",
    fill = NULL
...
```{r dif-vs-ex-score-usa-men, fig.cap = "Difference between Difficulty and Exectution Scores for team USA Men"}
usa_men_pivot_longer <- pivot_longer(m_team_usa, cols = c(D_Score, E_Score), names_to = "D_E_labs", values_to = "D_E_Score")

ggplot(data = usa_men_pivot_longer, aes(x = clean_name, y = D_E_Score, fill = D_E_labs))+
  geom_bar(stat = "identity",
           position = position_dodge(width = 0.8))+
  theme_light()+
  scale_fill_aaas(labels = c("Difficulty Score", "Execution Score"))+  
