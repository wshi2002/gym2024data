...
```{r, include = FALSE}
# load in libraries
package_list <- c("tidyverse", "vroom", "stringr", "dplyr", "ggsci", "gridExtra", "cowplot")
...
cat("\\hline\n")
cat("\\multicolumn{5}{c}{Average Weighted Medal Count} \\\\\n")
cat("\\hline\n")
cat("\\textit{5.9} & \\textit{5.4} & \\textit{4.5} & \\textit{4.8} & \\textit{6.2} \\\\\n")
cat("\\hline\n")
...
cat("Frederick Richard & Colt Walker & Khoi Young & Khoi Young & Khoi Young \\\\\n")
cat("\\hline\n")
cat("\\multicolumn{5}{c}{Percentage of Simulations With USA Having the Highest Weighted Medal Count} \\\\\n")
cat("\\hline\n")
cat("\\textit{6.7\\%} & \\highlight{\\textit{15.7\\%}} & \\textit{8.7\\%} & \\textit{3.3\\%} & \\textit{13.7\\%} \\\\\n")
cat("\\hline\n")
cat("\\multicolumn{5}{c}{Average Weighted Medal Count} \\\\\n")
cat("\\hline\n")
cat("\\textit{4.4} & \\textit{6.1} & \\textit{5.7} & \\textit{4.3} & \\textit{5.4} \\\\\n")
cat("\\end{tabular}\n")
cat("\\end{table}\n")

```

As seen in the table above, Team USA is taken to be Asher Hong, Patrick Hoopes, Paul Juda, Brody Malone, and Colt Walker (Team 7 above). This Team USA won the highest weighted medal count of any country 15.7% of time during the second round simulations. The average weighted medal count from these simulation rounds was also included in this table. Although Team USA was chosen from the highest weighted medal count due to the importance of selecting the team that can best compete against the other countries in the Olympics, Team 7 also had the second-highest average weighted medal count of the top 10 men's teams. This further provides evidence that the team can perform consistently well in medal events. 
...
\begin{tabular}{c|c|c}
  ...
  \hline
  \textit{16.8} & \textit{17.5} & \highlight{\textit{18.4}} \\
\end{tabular}
\end{center}
...
Additionally, we created heatmaps of the average country performance across the 300 simulations with team USA held constant and with the selected athletes from Results. Figure 2, below, shows that for men's gymnastics, individual athletes who do not belong to a qualifying country tend to dominate events such as Pommel Horse, Floor Exercise, Vault, and Still Rings. However, this could be unrealistic in the Olympics, as individual athletes still have to qualify in order to compete. Our current simulation allows for all individuals not part of a qualifying country and who passed our athlete selection criteria to compete in the individual medal events, regardless of whether it is probable for them to qualify in the real-world Olympic Games. For our selected USA men's team, Team All-Around and Individual All-Around appear to have the highest average weighted medal count for the USA. The heatmap also shows that China and Japan are strong competitors. China and Japan have historically dominated past Olympic men gymnastic events [6]. 

Figure 2 shows that for women's gymnastics, our selected USA team tends to dominate most events, except uneven bars. USA's relative weakness in uneven bars is consistent historical performances, as the USA has not won gold in Uneven Bars at the Olympics since 1984 [7]. USA has a high average weighted medal count in the Individual All-Around event in this simulation, and has won gold in women's Individual All-Around at every single Olympics since 2004 [8]. Although the data in this simulation only included information from 2022 and 2023 competitions, it is interesting to compare the results of the simulation to past real-world Olympic outcomes.
...
plot_grid(heatmap_men, heatmap_women, align = "v",nrow = 2, rel_heights = c(1/3, 1/2))
```
...
More generally, since our method included taking the median of the kernel densities of each athlete and apparatus for team selection, the median may not provide a comprehensive summary of the data if there are only a small number of data points for an athlete. As well, due to our method of creating a continuous distribution from the observed scores of the athletes, the simulation did not truncate the potential scores that athletes could receive at the maximum possible score for each apparatus. The score distributions for each athlete and apparatus potentially allow for technically impossible scores (such as a simulated score higher than the highest possible real-world score), albeit at a low probability.Additionally, our simulation did not weight scores based on recency, which does not take into account if athletes experienced a growth trajectory during the 2022-2023 season -  who may have ranked poorly in the beginning competitions but are beginning to perform better -  had kernel densities that weighted their poor rankings the same as their more recent high rankings. More qualitative data about each athlete could also be important to include. For example, an athlete's future plans or competition data from past Olympics were not included; it could make sense to include whether a past Olympian is defending his/her title, or whether this is someone's last planned Olympics, as determining factors if two athletes have otherwise similar performance history. It could also be important to have feedback from coaches about the mental and physical readiness of each athlete. Including these additional variables could have added nuance to our selection process.

Our method of simulation included assumptions that defined what a "best performance" by Team USA meant. Particularly, the weighting of medals (with gold = 3, silver = 2, and bronze = 1) assumed an increase in prestige that could differ from other interpretations. If the United States is only interested in the total number of people on the podium, for example, a bronze medal may be weighted the same as a gold medal. Another assumption made was that each event medal was equally prestigious; a "gold" in vault was weighted the same as a "gold" in the floor exercise. Depending on the priorities or strategies of the USA Gymnastics, they may see certain events as more important to win (for example to defend a title), in which case it would be appropriate to apply a different level of weighting for weighted random sampling of teams. For example, if team USA wants to focus on winning gold for uneven bars, the weights could then be biased towards gymnasts who compete in this event. These alternative methods could be useful depending on the specific goals of USA Gymnastics.In addition to alternative methods for selecting individual athletes, there are many different ways to select the best teams. We chose to select teams based on the highest percentage of times they had the highest  weighted medal count  as compared to all other competing countries. However, there are cases where this may not be the best metric to use. As an extreme example, it is possible that we selected “Team A” when it had a  20% chance of getting 10 medals (the highest) but an 80% chance of getting 0 medals, whereas Team B had a 19% chance of getting 10 medals but an 81% chance of getting 9 medals. Using an “average weighted medal count” metric as part of the team-choosing analysis could address this limitation.  In our women’s results, we are choosing the final team with the highest expected medal count.  This is used as our tiebreaker for our final simulation analysis. Our men’s results table includes a row that has the average weighted medal count of the top 10 teams. Although we selected the team that had the highest weighted medal count the most number of times, the team we selected also had the second-highest average weighted medal count. This addresses the limitation that they could have had very poor performance for the times they did not win the highest weighted medal count. These alternative metrics could be useful depending on what USA Gymnastics wants. Do they want the highest overall medal counts? Do they want to be the best relative to other countries? These are important questions especially for USA men’s gymnastics, as historically medal counts have been low for them. 
...
[17] Grube, Riley "2024 Olympic Gymnastics Qualification Predictions." wsportsxchange.com, 28 Sept. 2023 https://www.wsportsxchange.com/post/2024-olympic-gymnastics-qualification-predictions.

[18] "Judging the judges – how statistical analysis evaluates fairness and accuracy in gymnastics scoring". Olympics, https://olympics.com/en/news/how-statistical-analysis-evaluates-fairness-accuracy-gymnastics. Accessed 2 Dec. 2023.

[19] "THE PREDICTION OF ALL-AROUND EVENT FINAL SCORE BASED ON D AND E SCORE FACTORS IN WOMEN ARTISTIC GYMNASTICS". ResearchGate, https://www.researchgate.net/publication/349519419. Accessed 2 Dec. 2023.
...
top_countries <- rbind(top_countries_women, top_countries_men)

top_countries <- top_countries %>% mutate(Gender = case_when (
  Gender == 'm' ~ 'Men',
  Gender == 'w' ~ 'Women'
))
...
```{r}
ggplot(data = top_countries, aes(y = clean_country, x = Rank))+
  ...
  labs(
    x = "Competition Rank",
    y = "Country",
    title = "Appendix 1: Each Athlete's Competition Ranks for Olympic Qualified Countries"
  )
```

```{r}

df_penalties <- rbind(df_women_c, df_men_c)

df_penalties <- df_penalties %>% mutate(Gender = case_when (
  Gender == 'm' ~ 'Men',
  Gender == 'w' ~ 'Women'
))
